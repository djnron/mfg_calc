...
async function loadData() {
  const [benchmarkResponse, catalogResponse] = await Promise.all([
    fetch(chrome.runtime.getURL("benchmarks.json")),
    fetch(chrome.runtime.getURL("product_estimates.json"))
  ]);
  benchmarks = await benchmarkResponse.json();
...
function rebuildArchetypes() {
  ...
  $("archetype").innerHTML = rows.map(x => `<option value="${escapeHtml(x.product_archetype)}">${escapeHtml(x.product_archetype)}</option>`).join("");
}
function chooseBenchmark(text) {
  ...
  for (const row of benchmarks) {
    ...
    const words = source.split(/\W+/).filter(x => x.length > 3);
    const s = words.reduce((sum, word) => sum + (text.includes(word) ? 1 : 0), 0);
...
function findCatalogMatch(product) {
  const pageUrl = normalizeUrl(product.url);
  if (pageUrl) {
    const exactUrl = productCatalog.find(x => x.product_url && normalizeUrl(x.product_url) === pageUrl);
    if (exactUrl) return exactUrl;
  }
  const title = normalizeTitle(product.title);
  if (!title || title.length < 8) return null;
  const exactTitle = productCatalog.find(x => normalizeTitle(x.title) === title);
...
function calculate() {
  ...
  const matched = catalogMatch && catalogMatch.product_archetype === row.product_archetype
    ? `Matched to the ${catalogMatch.platform} reference catalog. `
    : "";
  $("notes").textContent = `${matched}${row.construction_summary}. ${row.notes} Quantity adjustment: ${vf.toFixed(2)}× versus the reference quantity of ${row.reference_quantity}.`;

  lastEstimate = {
    captured_at:new Date().toISOString(),
    product_title:$("title").value,
    page_url:$("title").dataset.url || "",
  ...
  chrome.storage.local.set({ lastEstimate });
}
async function capture() {
  const [tab] = await chrome.tabs.query({ active:true, currentWindow:true });
  if (!tab?.id) return;
  const response = await chrome.tabs.sendMessage(tab.id, { type:"EXTRACT_PRODUCT" }).catch(() => null);
  if (!response?.product) return;
  const p = response.product;
  $("title").value = p.title || "";
  $("title").dataset.url = p.url || "";
  $("retailPrice").value = p.retail_price || "";
  catalogMatch = findCatalogMatch(p);
  if (catalogMatch) applyCatalogMatch(catalogMatch);
  else chooseBenchmark(`${p.title} ${p.description}`);
  calculate();
}
...
function exportCSV() {
  ...
  chrome.downloads.download({ url, filename:"manufacturing_cost_estimate.csv", saveAs:true });
}
...
document.querySelectorAll("input,select").forEach(el => el.addEventListener("change", () => {
  if (el.id !== "category" && el.id !== "confidenceOverride") calculate();
}));
loadData().then(async () => {
  const saved = await chrome.storage.local.get("lastEstimate");
  if (saved.lastEstimate) {
    $("title").value = saved.lastEstimate.product_title || "";
    $("retailPrice").value = saved.lastEstimate.retail_price_usd || "";
    $("quantity").value = saved.lastEstimate.quantity || 1000;
  }
  capture();
});